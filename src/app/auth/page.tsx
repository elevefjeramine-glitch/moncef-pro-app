...
// Les quatre logos des boutons OAuth, définis au niveau module et non dans le corps de
// `AuthPage`. Tels qu ils etaient écrits, ils changeaient d identite a chaque rendu :
// React demonvait et remontait le <svg> a chaque frappe dans un champ. Regle
// `react-hooks/static-components` : 4 erreurs avant, 0 apres (et le HTML rendu est
// identique octet pour octet, verifie sur le build servi).
...
export default function AuthPage() {
  ...
  useEffect(() => {
    ...
    // Ce `setLang` synchrone dans un effet est bien ce que la règle signale, et je ne le
    // déplace pas dans l'initialiseur de `useState` : `localStorage` n'existe pas au
    // rendu serveur, donc lire la langue au premier render côté client produirait un HTML
    // différent de celui du serveur — une erreur d'hydratation sur la page de connexion,
    // c'est-à-dire sur la porte d'entrée. Un rendu de trop au montage est le bon échange.
