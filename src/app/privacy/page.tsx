...
const content = {
  ...
  zh: {
    title: "隐私政策",
    subtitle: "2026-08-28 版本。以下每一条陈述都对应这一天服务的真实状态。",
    intro: "本页替换了一段写错的旧文本：它承诺了服务并未实现的端到端加密，也不存在所谓“在本地重新分析”。下面是平台对你数据的实际处理方式。",
    note: "本文件描述的是一个软件，不构成法律意见；若服务扩展到其他学校或开始收费，请交由专业人士复核。",
    back: "返回首页",
    sections: [
      { icon: "Eye", title: "我们保存什么", body: [
        "注册时：邮箱地址、名字与姓氏（若通过 Google 或 Microsoft 登录，则取自该账户）、角色（normal、moderator 或 founder）、额度余额、主题颜色与界面语言。",
        "你在应用中创建的内容：作业（科目、题目、教师、优先级、状态、进度、截止日期）、课程表条目（A/B 周、星期、科目、时间段）、事件（标题、描述、日期、时间、类别）以及站内消息。",
        "账户创建时间。仅此而已：应用不记录请求日志，不采集浏览器指纹，也不在服务端保留草稿。",
      ]},
      { icon: "Server", title: "数据存放在哪里", body: [
        "数据库是 Supabase 项目，位于 us-east-2 区域，即美国俄亥俄州。它不在欧洲。",
        "站点页面由 Netlify 通过其全球分发网络提供服务。",
        "对 RGPD 的实际含义：这是一次向欧盟以外国家的数据传输。我们自身没有标准合同条款，合法性依据建立在用户对该服务的使用之上；如需要，仍可将项目迁移到欧洲区域。",
      ]},
      { icon: "Lock", title: "加密：真实情况", body: [
        "你的浏览器与服务器之间的链路使用 HTTPS（TLS）加密，包括 API 的所有请求。",
        "这不是端到端加密。数据在数据库中是明文可读的——服务运营方可以读，作为托管方的 Supabase 在技术上也能读。没有启用任何额外的静态加密，也不存在我们读不了的密钥。",
        "密码本身从不以明文保存：由认证服务做哈希处理。",
      ]},
      { icon: "Users", title: "AI 会看到你发送的内容", body: [
        "每一段对话都会发送给外部语言模型：主用 Google 的 Gemini，故障时改用 Groq。回答既不在你的浏览器里生成，也不在我们的服务器上生成。",
        "我们不会转卖这些数据，也不用它们训练自有模型。但处理过程遵循所用供应商的条款，这一点我们无法控制。",
        "不要把你不愿交给这两家公司的内容粘贴进对话框。",
      ]},
      { icon: "Database", title: "我们不做什么", body: [
        "没有广告 Cookie，没有跟踪器，页面里不加载任何第三方脚本：浏览器中唯一被记住的是站点语言（本地存储）。",
        "不收费：服务免费，我们不经手银行卡信息，也不收集邮寄地址。",
        "不转卖数据，不发营销邮件，没有订阅通讯。",
      ]},
      ...
      { icon: "Baby", title: "未成年学生账户", body: [
        "本服务面向学生，其中多数未成年。账户绑定一个邮箱：预期由学生本人查看该邮箱，并依适用规则取得法定监护人的同意。",
        "moderator 或 founder 可通过管理面板查看账户列表及其元数据，也可删除账户：这一点并不隐瞒，该面板的存在正是为此。",
      ]},
      { icon: "MailQuestion", title: "联系我们与你的权利", body: [
        "访问、更正、异议或投诉：请通过学校的渠道，或在应用内向管理员反馈。本站有意不公布任何邮箱地址，以免运营方的地址被自动抓取。",
        "你无需向我们申请即可获得自己数据的副本：消息、作业与课程表都属于你，并在你的账户中展示。",
        "若未得到回应，你可向所在国的数据保护机构提出投诉。",
      ]},
    ],
  },
};
